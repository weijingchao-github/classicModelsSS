...
class ResNet(nn.Module):
    ...
    def __int__(self, block, layer, num_classes=1000, zero_init_residule=False,
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if dilate:
            ...
            stride = 1
        if stride != 1 or self.inplanes != planes * block.expantion:
            downsample = nn.Sequential(
                conv1x1(self.inplanes, planes * block.expantion, stride),
                norm_layer(planes * block.expantion),
            )
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer))
        self.inplanes = planes * block.expantion
